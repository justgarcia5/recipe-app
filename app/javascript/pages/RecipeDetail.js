...
const RecipeDetail = props => {
  ...
  useEffect(() => {
    fetch(
      `https://api.edamam.com/search?q=${props.match.params.label}&app_id=${APP_ID}&app_key=${APP_KEY}`
    )
      .then(response => response.json())
      .then(data => {
        let recipes = data.hits.map(hit => hit.recipe);
        let filteredRecipeNames = recipes.filter(
          recipe => recipe.label === props.match.params.label
        );
        let filteredRecipes = filteredRecipeNames.filter(
          (recipe, index) => index === 0
        );
          ...
          <div key={index}>
            ...
            <div className="detail-page-row-1">
              <div>
                ...
                />
                <p>
                  <i>Source: </i> {recipe.source}
                </p>
                <p>
                  <i>Total Calories: </i> {Math.floor(recipe.calories)}
                </p>
                {props.currentUser && (
                  <FavoritesSubmit
                    recipe={recipe}
                  ...
                  return (
                    <div key={index}>
                      <ul>
                        ...
                        <li className="recipe-ingredients-weight">
                          Weight: {Math.floor(ingredient.weight)} grams
                        </li>
                      </ul>
                    </div>
                  );
